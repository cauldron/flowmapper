...
def test_string_list_empty():
    ...
    assert (
        list(iter(sl)) == []
    ), f"Expected list(iter(sl)) to be [], but got {list(iter(sl))}"
    assert len(sl) == 0, f"Expected len(sl) to be 0, but got {len(sl)}"
    assert not sl, f"Expected sl to be falsy, but got {sl}"
    assert (
        repr(sl) == "StringList: Empty"
    ), f"Expected repr(sl) to equal 'StringList: Empty', but got {repr(sl)!r}"
...
def test_string_list_no_transformed():
    ...
    assert list(iter(sl)) == [
        "a",
        "b",
    ], f"Expected list(iter(sl)) to equal ['a', 'b'], but got {list(iter(sl))}"
    assert (
        sl.data[0].original == "A"
    ), f"Expected sl.data[0].original to be 'A', but got {sl.data[0].original!r}"
    assert (
        sl.data[0].normalized == "a"
    ), f"Expected sl.data[0].normalized to be 'a', but got {sl.data[0].normalized!r}"
