...
def test_equals_mass():
    ...
    assert u1 == u2, f"Expected u1 to equal u2, but they are not equal (u1={u1!r}, u2={u2!r})"


def test_energy():
    u1 = UnitField("kilowatt hour")
    u2 = UnitField("MJ")
    assert u1.compatible(u2), f"Expected u1 to be compatible with u2, but they are not (u1={u1!r}, u2={u2!r})"
    assert u1.conversion_factor(u2) == 3.6, f"Expected u1.conversion_factor(u2) to be 3.6, but got {u1.conversion_factor(u2)}"
...
def test_enrichment():
    ...
    assert u1.compatible(u2), f"Expected u1 to be compatible with u2, but they are not (u1={u1!r}, u2={u2!r})"
    assert u1.conversion_factor(u2) == 1e-3, f"Expected u1.conversion_factor(u2) to be 1e-3, but got {u1.conversion_factor(u2)}"
...
def test_livestock():
    ...
    assert u1.normalize() == u2.normalize(), f"Expected u1 to equal u2, but they are not equal (u1={u1!r}, u2={u2!r})"
...
def test_complex_conversions():
    ...
    assert u1.conversion_factor(u2) == 0.012, f"Expected u1.conversion_factor(u2) to be 0.012, but got {u1.conversion_factor(u2)}"
...
class TestUnitFieldNormalize:
    ...
    def test_normalize_with_valid_unit(self):
        ...
        assert normalized == "kilogram", f"Expected normalized to be 'kilogram', but got {normalized!r}"
        assert isinstance(normalized, UnitField), f"Expected normalized to be a UnitField instance, but got {type(normalized)}"
...
class TestUnitFieldEq:
    ...
    def test_eq_with_different_units(self):
        ...
        u1 = UnitField("kg")
        u2 = UnitField("g")
        assert u1 != u2, f"Expected u1 to not equal u2, but they are equal (u1={u1!r}, u2={u2!r})"
...
class TestUnitFieldCompatible:
    ...
    def test_compatible_with_incompatible_units(self):
        ...
        u1 = UnitField("kg")
        u2 = UnitField("meter")
        assert not u1.compatible(u2), f"Expected u1 to not be compatible with u2, but they are (u1={u1!r}, u2={u2!r})"
    ...
    def test_compatible_with_non_unitfield(self):
        ...
        u1 = UnitField("kg")
        # Should return False for non-UnitField types
        assert not u1.compatible("kg"), f"Expected u1 to not be compatible with 'kg' string, but it is (u1={u1!r})"
        assert not u1.compatible(123), f"Expected u1 to not be compatible with 123, but it is (u1={u1!r})"
...
class TestUnitFieldConversionFactor:
    ...
    def test_conversion_factor_with_non_unitfield(self):
        ...
        assert math.isnan(result), f"Expected conversion_factor to be NaN for non-UnitField, but got {result}"
    ...
    def test_conversion_factor_with_undefined_unit(self):
        ...
        result = u1.conversion_factor(u2)
        assert math.isnan(result), f"Expected conversion_factor to be NaN for undefined unit, but got {result}"
    ...
    def test_conversion_factor_with_dimensionality_error(self):
        ...
        u2 = UnitField("meter")
        result = u1.conversion_factor(u2)
        assert math.isnan(result), f"Expected conversion_factor to be NaN for incompatible units, but got {result}"
    ...
    def test_conversion_factor_zero_to_one(self):
        ...
        u2 = UnitField("kg")
        result = u1.conversion_factor(u2)
        assert result == 1e-06, f"Expected conversion_factor to be 1e-06, but got {result}"
    ...
    def test_conversion_factor_one_to_zero(self):
        ...
        u2 = UnitField("mg")
        result = u1.conversion_factor(u2)
        assert result == 1e06, f"Expected conversion_factor to be 1e06, but got {result}"
