...
class TestOxidationStateInitialization:
    ...
    def test_init_with_boundary_values(self):
        ...
        assert os_min.value == -5, f"Expected os_min.value to be -5, but got {os_min.value}"
        assert os_max.value == 9, f"Expected os_max.value to be 9, but got {os_max.value}"
...
class TestOxidationStateEq:
    ...
    def test_eq_with_same_oxidation_state(self):
        ...
        assert os1 == os2, f"Expected os1 to equal os2, but they are not equal (os1={os1.value}, os2={os2.value})"

    def test_eq_with_different_oxidation_state(self):
        """Test equality with different OxidationState."""
        os1 = OxidationState(3)
        os2 = OxidationState(4)
        assert os1 != os2, f"Expected os1 to not equal os2, but they are equal (os1={os1.value}, os2={os2.value})"

    def test_eq_with_integer(self):
        """Test equality with integer."""
        os = OxidationState(3)
        assert os == 3, f"Expected os to equal 3, but they are not equal (os={os.value})"
        assert os != 4, f"Expected os to not equal 4, but they are equal (os={os.value})"

    def test_eq_with_negative_integer(self):
        """Test equality with negative integer."""
        os = OxidationState(-2)
        assert os == -2, f"Expected os to equal -2, but they are not equal (os={os.value})"
        assert os != -3, f"Expected os to not equal -3, but they are equal (os={os.value})"

    def test_eq_with_zero(self):
        """Test equality with zero."""
        os = OxidationState(0)
        assert os == 0, f"Expected os to equal 0, but they are not equal (os={os.value})"
        assert os != 1, f"Expected os to not equal 1, but they are equal (os={os.value})"
...
class TestOxidationStateHasOxidationState:
    ...
    def test_has_oxidation_state_with_roman_numeral_lowercase(self):
        ...
        assert OxidationState.has_oxidation_state("chromium (iii)"), "Expected has_oxidation_state('chromium (iii)') to return True, but it returned False"
        assert OxidationState.has_oxidation_state("iron (ii)"), "Expected has_oxidation_state('iron (ii)') to return True, but it returned False"
        assert OxidationState.has_oxidation_state("manganese (vi)"), "Expected has_oxidation_state('manganese (vi)') to return True, but it returned False"

    def test_has_oxidation_state_with_roman_numeral_uppercase(self):
        """Test has_oxidation_state with uppercase roman numeral."""
        assert OxidationState.has_oxidation_state("Iron (II)"), "Expected has_oxidation_state('Iron (II)') to return True, but it returned False"
        assert OxidationState.has_oxidation_state("Chromium (III)"), "Expected has_oxidation_state('Chromium (III)') to return True, but it returned False"
        assert OxidationState.has_oxidation_state("Mercury (IV)"), "Expected has_oxidation_state('Mercury (IV)') to return True, but it returned False"
    ...
    def test_has_oxidation_state_with_number(self):
        ...
        assert OxidationState.has_oxidation_state("iron (2)"), "Expected has_oxidation_state('iron (2)') to return True, but it returned False"
        assert OxidationState.has_oxidation_state("iron (3+)"), "Expected has_oxidation_state('iron (3+)') to return True, but it returned False"
        assert OxidationState.has_oxidation_state("iron (2-)"), "Expected has_oxidation_state('iron (2-)') to return True, but it returned False"
    ...
    def test_has_oxidation_state_with_number_no_parentheses(self):
        ...
        assert OxidationState.has_oxidation_state("iron -2"), "Expected has_oxidation_state('iron -2') to return True, but it returned False"
        assert OxidationState.has_oxidation_state("iron 2-"), "Expected has_oxidation_state('iron -2') to return True, but it returned False"
        assert OxidationState.has_oxidation_state("iron 02-"), "Expected has_oxidation_state('iron -2') to return True, but it returned False"

    def test_has_oxidation_state_without_oxidation_state(self):
        """Test has_oxidation_state without oxidation state."""
        assert not OxidationState.has_oxidation_state("water"), "Expected has_oxidation_state('water') to return False, but it returned True"
        assert not OxidationState.has_oxidation_state("iron"), "Expected has_oxidation_state('iron') to return False, but it returned True"
        assert not OxidationState.has_oxidation_state("chromium oxide"), "Expected has_oxidation_state('chromium oxide') to return False, but it returned True"
...
class TestOxidationStateFromString:
    ...
    def test_from_string_with_roman_numeral_lowercase(self):
        ...
        assert os.value == 3, f"Expected os.value to be 3, but got {os.value}"
        assert remaining == "chromium", f"Expected remaining to be 'chromium', but got {remaining!r}"
    ...
    def test_from_string_with_roman_numeral_uppercase(self):
        ...
        assert os.value == 2, f"Expected os.value to be 2, but got {os.value}"
        assert remaining == "Iron", f"Expected remaining to be 'Iron', but got {remaining!r}"
    ...
    def test_from_string_with_comma_and_leading_zeros(self):
        ...
        assert os.value == 2, f"Expected os.value to be 2, but got {os.value}"
        assert remaining == "foo", f"Expected remaining to be 'foo', but got {remaining!r}"
    ...
    def test_from_string_raises_error_both_signs(self):
        ...
        with pytest.raises(ValueError, match="Sign before and after"):
    ...
    def test_from_string_raises_error_values_outside_bounds_numbers(self):
        ...
        with pytest.raises(ValueError, match="physically impossible"):
            OxidationState.from_string("iron (-10)")
        with pytest.raises(ValueError, match="physically impossible"):
            OxidationState.from_string("iron (6-)")  # -6
    ...
    def test_from_string_various_roman_numerals(self):
        ...
        for string, expected_value in test_cases:
            ...
            assert os.value == expected_value, f"Expected os.value to be {expected_value} for '{string}', but got {os.value}"
    ...
    def test_from_string_remaining_string(self):
        ...
        for string, expected_remaining in test_cases:
            os, remaining = OxidationState.from_string(string)
            assert remaining == expected_remaining, f"Expected remaining to be {expected_remaining!r} for '{string}', but got {remaining!r}"
