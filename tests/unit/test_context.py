...
from flowmapper.context import ContextField, MISSING_VALUES
...
class TestContextFieldInitialization:
    ...
    def test_init_with_string(self):
        ...
        assert c.value == "Raw/(unspecified)", f"Expected c.value to be 'Raw/(unspecified)', but got {c.value!r}"
        assert isinstance(c.value, str), f"Expected c.value to be a str, but got {type(c.value)}"

    def test_init_with_list(self):
        """Test initialization with list."""
        c = ContextField(["Raw", "(unspecified)"])
        assert c.value == ["Raw", "(unspecified)"], f"Expected c.value to be ['Raw', '(unspecified)'], but got {c.value!r}"
        assert isinstance(c.value, list), f"Expected c.value to be a list, but got {type(c.value)}"

    def test_init_with_tuple(self):
        """Test initialization with tuple."""
        c = ContextField(("Raw",))
        assert c.value == ("Raw",), f"Expected c.value to be ('Raw',), but got {c.value!r}"
        assert isinstance(c.value, tuple), f"Expected c.value to be a tuple, but got {type(c.value)}"
...
class TestContextFieldNormalize:
    ...
    def test_normalize_with_string(self):
        ...
        assert normalized.value == ("a", "b"), f"Expected normalized.value to be ('a', 'b'), but got {normalized.value!r}"
        assert isinstance(normalized.value, tuple), f"Expected normalized.value to be a tuple, but got {type(normalized.value)}"
        assert c.value == "A/B", f"Expected original c.value to remain 'A/B', but got {c.value!r}"
    ...
    def test_normalize_with_list(self):
        ...
        c = ContextField(["Raw", "(unspecified)"])
        normalized = c.normalize()
        assert normalized.value == ("raw",), f"Expected normalized.value to be ('raw',), but got {normalized.value!r}"
    ...
    def test_normalize_with_obj_parameter(self):
        ...
        assert normalized.value == ("a", "b"), f"Expected normalized.value to be ('a', 'b'), but got {normalized.value!r}"
        assert c.value == "X/Y", f"Expected original c.value to remain 'X/Y', but got {c.value!r}"
    ...
    def test_normalize_removes_trailing_missing_values(self):
        ...
        normalized = c.normalize()
        assert normalized.value == ("a",), f"Expected normalized.value to be ('a',), but got {normalized.value!r}"
    ...
    def test_normalize_removes_trailing_missing_value(self, missing_value):
        ...
        normalized = c.normalize()
        assert normalized.value == ("a",), f"Expected normalized.value to be ('a',) for missing value {missing_value!r}, but got {normalized.value!r}"
    ...
    def test_normalize_does_not_remove_leading_missing_values(self):
        ...
        normalized = c.normalize()
        assert normalized.value == ("(unknown)", "a"), f"Expected normalized.value to be ('(unknown)', 'a'), but got {normalized.value!r}"

    def test_normalize_returns_new_instance(self):
        """Test that normalize returns a new instance."""
        c = ContextField("A/B")
        normalized = c.normalize()
        assert normalized is not c, "Expected normalize() to return a new instance, but it returned the same instance"
        assert c.value == "A/B", f"Expected original c.value to remain 'A/B', but got {c.value!r}"
...
class TestContextFieldExportAsString:
    ...
    def test_export_as_string_with_list(self):
        ...
        assert result == "A✂️B", f"Expected export_as_string() to be 'A✂️B', but got {result!r}"
    ...
    def test_export_as_string_with_string(self):
        ...
        c = ContextField("A/B")
        result = c.export_as_string()
        assert result == "A/B", f"Expected export_as_string() to be 'A/B', but got {result!r}"
    ...
    def test_export_as_string_with_custom_join_character_tuple(self):
        ...
        result = c.export_as_string("|")
        assert result == "A|B|C", f"Expected export_as_string('|') to be 'A|B|C', but got {result!r}"
    ...
    def test_export_as_string_with_custom_join_character_string_value(self):
        ...
        assert result == "A/B", f"Expected export_as_string('/') to be 'A/B' for string value, but got {result!r}"
    ...
    def test_export_as_string_with_custom_join_character_empty_string(self):
        ...
        c = ContextField(["A", "B"])
        result = c.export_as_string("")
        assert result == "AB", f"Expected export_as_string('') to be 'AB', but got {result!r}"
...
class TestContextFieldEq:
    ...
    def test_eq_with_same_contextfield(self):
        ...
        c1 = ContextField("A/B")
        c2 = ContextField("A/B")
        assert c1 == c2, f"Expected c1 to equal c2, but they are not equal (c1={c1!r}, c2={c2!r})"

    def test_eq_with_different_contextfield(self):
        """Test equality with different ContextField."""
        c1 = ContextField("A/B")
        c2 = ContextField("X/Y")
        assert c1 != c2, f"Expected c1 to not equal c2, but they are equal (c1={c1!r}, c2={c2!r})"
    ...
    def test_eq_with_string_other(self):
        ...
        assert c != "A/B", f"Expected c to not equal 'A/B', but they are equal (c={c!r})"
    ...
    def test_eq_with_empty_contextfield(self):
        ...
        # But normalizing empty string raises IndexError when accessing intermediate[-1]
...
class TestContextFieldHash:
    ...
    def test_hash_with_string(self):
        ...
        c = ContextField("A/B")
        result = hash(c)
        assert isinstance(result, int), f"Expected hash(c) to be an int, but got {type(result)}"
    ...
    def test_hash_same_values(self):
        ...
        c1 = ContextField("A/B")
        c2 = ContextField("A/B")
        assert hash(c1) == hash(c2), f"Expected hash(c1) to equal hash(c2), but got {hash(c1)} and {hash(c2)}"
...
class TestContextFieldIter:
    ...
    def test_iter_with_string(self):
        ...
        c = ContextField("A/B")
        result = list(c)
        assert result == ["A", "/", "B"], f"Expected list(c) to be ['A', '/', 'B'], but got {result!r}"
...
class TestContextFieldContains:
    ...
    def test_contains_with_string_values(self):
        ...
        # "A" == "A/B"[:1] -> "A" == "A" -> True
        assert c2 in c1, f"Expected c2 to be in c1, but it was not (c1={c1!r}, c2={c2!r})"
        assert c1 not in c2, f"Expected c1 to not be in c2, but it was (c1={c1!r}, c2={c2!r})"
...
class TestContextFieldRepr:
    ...
    def test_repr_with_string(self):
        ...
        c = ContextField("A/B")
        result = repr(c)
        assert result == "ContextField: A/B", f"Expected repr(c) to be 'ContextField: A/B', but got {result!r}"
...
class TestContextFieldEdgeCases:
    ...
    def test_normalize_preserves_original_value(self):
        ...
        normalized = c.normalize()
        assert c.value == "ORIGINAL", f"Expected original c.value to remain 'ORIGINAL', but got {c.value!r}"
